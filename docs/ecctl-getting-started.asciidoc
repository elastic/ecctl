...
As a first example of using {p}, let's run the <<ecctl_deployment_list,{p} deployment list>> command to retrieve information about existing deployments. This is a good way to confirm that {p} is configured correctly. If you have any deployments already created, they'll be listed in the results.
...
Let's create a basic deployment. Elasticsearch Service supports {cloud}/ec-getting-started-templates.html[deployment templates], which pre-configure the Elastic Stack components in your deployment to best suit your particular use case. For this example, we'll use Google Cloud Platform (GCP) to host the deployment in region `US Central 1 (Iowa)`. For details about what deployment options are available by platform, see our list of {cloud}/ec-regions-templates-instances.html[available regions, deployment templates and instance configurations].
...
<5> The template on which to base the deployment
<6> The number of availability zones for Kibana
<7> The amount of memory allocated for Kibana
<8> The number of availability zones for APM
<9> The amount of memory allocated for APM
...
The response indicates that the request was submitted successfully. It includes the `elastic` user password, which you can use to log in to Kibana or to access the Elasticsearch REST API. Make a note of the deployment ID, which we will use in the next example.

[id="{p}-example-update-deployment"]
=== Update a deployment

Now that you have used {p} to create a deployment, let's update it. We will scale it up, increasing the size of the Elasticsearch data nodes from 4096 to 8192 MB.
...
--
where `$DEPLOYMENT_ID` is the ID for the deployment that was created in the previous <<{p}-example-create-deployment,create a deployment>> example.
