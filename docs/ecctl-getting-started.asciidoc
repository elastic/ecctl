...
To upgrade {p} via brew, it's very simple to do so:
...
Updated 1 tap (elastic/tap).
No changes to formulae.
==> Upgrading ecctl from elastic/tap
...
==> Caveats
...
==> Summary
🍺  /usr/local/Cellar/ecctl/1.0.0-beta2: 5 files, 22.7MB, built in 21 seconds
Removing: /usr/local/Cellar/ecctl/1.0.0-beta1... (5 files, 21.3MB)
----
